...
#' 
#' @description
#' This function expects a Client ID and Client Secret. In order to obtain these, you will
#' have to register an application with the Google Analytics API. This can be done as follows
#' 
...
#' In subsequent runs, User Consent is not required unless you are querying a Google Analytics profile
#' associated with a different email account. This function uses \code{\link[httr]{oauth2.0_token}} under the hood to create the OAuth Tokens.
#' The Access Token has a 60 minute lifetime after which it expires and a new token has to be obtained. 
#' This can be done using the \code{\link{ValidateToken}} method
#' 
#' @export  
#' 
#' @return google.token A Token object containing all the data required for OAuth access. See \code{\link[httr]{Token2.0}} for 
